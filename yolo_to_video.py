...
import cv2
import torch
from daal4py import preprocessing
import numpy as np
...
def main(): 
    ...
    while True:
        ...
        # oneDAL preprocessing
        preprocessed_frame = np.array(img)
        preprocessed_frame = preprocessed_frame.transpose((2, 0, 1)).astype(np.uint8)
        pca_data = preprocessing.PCATransform(nComponents=3).compute(preprocessed_frame)
        img_tensor = torch.from_numpy(pca_data).unsqueeze(0).to(device)
